...
# =============================================================
# DiagramBuilder — GitHub セットアップスクリプト v2
# Issue を Milestone なしで作成します
# 後から GitHub Web UI で Milestone を紐付けてください
# =============================================================
...
if [ -z "$OWNER" ] || [ -z "$REPO" ]; then
  echo "使い方: ./setup_github_v2.sh <GitHubユーザ名> <リポジトリ名>"
  echo "例:     ./setup_github_v2.sh nul-dev0 diagram-builder"
  exit 1
fi
...
create_issue "リポジトリ作成・初期設定（README, LICENSE, .gitignore）" "infra,docs" \
"## 概要
リポジトリの初期設定を行う。
...
create_issue ".env.example / config.example.json の作成" "security,infra" \
"## 概要
...
create_issue "フロントエンド: React + TypeScript プロジェクト初期化" "frontend" \
"## 概要
...
create_issue "バックエンド: Node.js + Express プロジェクト初期化" "backend" \
"## 概要
...
create_issue "ESLint + eslint-plugin-security の設定" "security,infra" \
"## 概要
...
create_issue "TypeScript 厳格モード（strict: true）の設定" "security,frontend,backend" \
"## 概要
...
create_issue "Husky + lint-staged でコミット前自動チェックを設定" "infra,security" \
"## 概要
...
create_issue "Tailwind CSS の導入・基本スタイル設定" "frontend" \
"## 概要
...
create_issue "フロントエンド・バックエンド間の開発用プロキシ設定" "frontend,backend" \
"## 概要
...
create_issue "GitHub Actions: npm audit 自動チェック CI を作成" "infra,security" \
"## 概要
...
create_issue "メイン画面レイアウトの実装（左パネル + 右パネル）" "frontend" \
"## 概要
...
create_issue "図の種類選択コンポーネントの実装（10種類）" "frontend" \
"## 概要
...
create_issue "要件入力フォームの実装" "frontend" \
"## 概要
...
create_issue "Mermaid.js の統合・リアルタイムプレビュー実装" "frontend" \
"## 概要
...
create_issue "CodeMirror エディタの統合（Mermaid コード編集）" "frontend" \
"## 概要
...
create_issue "プレビューのリアルタイム更新ロジックの実装" "frontend" \
"## 概要
...
create_issue "Chrome / Edge / Firefox / Safari でのブラウザ互換テスト" "frontend" \
"## 概要
4 ブラウザでの動作確認を行う。
...
create_issue "レスポンシブレイアウトの確認・調整" "frontend" \
"## 概要
...
create_issue "LLM プロバイダ抽象化レイヤーの設計・実装" "backend" \
"## 概要
...
create_issue "Claude API（Anthropic）の統合" "backend" \
"## 概要
...
create_issue "OpenAI API の統合" "backend" \
"## 概要
OpenAI API を統合する。
...
create_issue "Google Gemini API の統合" "backend" \
"## 概要
...
create_issue "Ollama（ローカル LLM）の統合" "backend" \
"## 概要
...
create_issue "Azure OpenAI の統合" "backend" \
"## 概要
...
create_issue "各図種別のプロンプトテンプレート作成（10種類）" "backend" \
"## 概要
...
create_issue "API 呼び出し前の送信データ確認ダイアログ実装" "frontend,security" \
"## 概要
...
create_issue "「再生成」ボタンの実装" "frontend,backend" \
"## 概要
...
create_issue "各プロバイダの動作確認テスト" "backend" \
"## 概要
...
create_issue "OS キーチェーン連携（keytar）の実装" "security,backend" \
"## 概要
...
create_issue ".env 経由での API キー管理ロジックの実装" "security,backend" \
"## 概要
...
create_issue "設定画面 UI の実装（プロバイダ選択・API キー入力）" "frontend" \
"## 概要
...
create_issue "テスト接続機能の実装" "frontend,backend" \
"## 概要
...
create_issue "セキュリティ設定 UI の実装" "frontend,security" \
"## 概要
...
create_issue "API キーがログに出力されないことを確認" "security,backend" \
"## 概要
...
create_issue "パストラバーサル対策の実装・テスト" "security,backend" \
"## 概要
...
create_issue "入力バリデーション（zod）の全エンドポイントへの適用" "security,backend" \
"## 概要
...
create_issue "GitHub Secret Scanning / Dependabot の有効化" "infra,security" \
"## 概要
...
create_issue "ローカルファイル保存機能の実装（JSON 形式）" "backend" \
"## 概要
...
create_issue "ブラウザ LocalStorage 保存機能の実装" "frontend" \
"## 概要
...
create_issue "両ストレージ間の同期ロジックの実装" "frontend,backend" \
"## 概要
...
create_issue "図一覧表示コンポーネントの実装" "frontend" \
"## 概要
...
create_issue "図の検索・タグフィルタ機能の実装" "frontend,backend" \
"## 概要
...
create_issue "図の読み込み・再編集機能の実装" "frontend,backend" \
"## 概要
...
create_issue "図の削除機能の実装" "frontend,backend" \
"## 概要
保存済みの図を削除できる機能を実装する。
...
create_issue "1000 件保存時のパフォーマンステスト" "backend" \
"## 概要
...
create_issue "Puppeteer / Chromium の導入・設定" "backend" \
"## 概要
...
create_issue "単一図の PDF 出力機能の実装" "backend" \
"## 概要
...
create_issue "複数図をまとめた PDF 出力機能の実装" "backend" \
"## 概要
...
create_issue "PDF 表紙（タイトル・日付・説明）の実装" "backend,frontend" \
"## 概要
PDF の先頭に表紙ページを追加する。
...
create_issue "A4 縦横対応の確認・調整" "backend" \
"## 概要
...
create_issue "PDF 出力ボタンの UI 実装" "frontend" \
"## 概要
...
create_issue "Electron プロジェクトのセットアップ" "electron" \
"## 概要
...
create_issue "Main Process / Renderer Process の構成設定" "electron" \
"## 概要
...
create_issue "preload.ts による安全な IPC 通信の実装" "electron,security" \
"## 概要
...
create_issue "nodeIntegration: false / contextIsolation: true の確認" "electron,security" \
"## 概要
...
create_issue "Content Security Policy（CSP）の設定" "electron,security" \
"## 概要
...
create_issue "electron-builder の設定（Mac .dmg / Windows .exe）" "electron,infra" \
"## 概要
...
create_issue "Mac 向けビルド・動作確認" "electron" \
"## 概要
...
create_issue "Windows 向けビルド・動作確認" "electron" \
"## 概要
...
create_issue "自動アップデート機能の実装（electron-updater）" "electron" \
"## 概要
...
create_issue "GitHub Releases へのビルド成果物アップロード" "infra,electron" \
"## 概要
...
create_issue "README.md の作成（インストール手順・セキュリティ注意事項含む）" "docs" \
"## 概要
...
create_issue "CONTRIBUTING.md の作成（コントリビュートガイド）" "docs" \
"## 概要
...
create_issue "CHANGELOG.md の作成" "docs" \
"## 概要
...
create_issue "全ブラウザ（Chrome/Edge/Firefox/Safari）最終テスト" "frontend" \
"## 概要
...
create_issue "Mac / Windows 両環境での最終動作確認" "electron" \
"## 概要
...
create_issue "npm audit 最終チェック・脆弱性ゼロの確認" "security,infra" \
"## 概要
...
create_issue "v1.0.0 タグの作成・GitHub Release の公開" "infra,docs" \
"## 概要
v1.0.0 を正式にリリースする。
...
echo ""
echo "  注意: Milestone との紐付けは GitHub Web UI で手動してください"
echo "================================================"
echo ""
