...
### Phase 4 — セキュリティ・設定管理 ✅ 完了
- [x] #29 OS キーチェーン連携（keytar）の実装
- [x] #30 .env 経由での API キー管理ロジックの実装
- [x] #31 設定画面 UI の実装（プロバイダ選択・API キー入力）
- [x] #32 テスト接続機能の実装
- [x] #33 セキュリティ設定 UI の実装
- [x] #34 API キーがログに出力されないことを確認
- [x] #35 パストラバーサル対策の実装・テスト
- [x] #36 入力バリデーション（zod）の全エンドポイントへの適用
- [x] #37 GitHub Secret Scanning / Dependabot の有効化

### Phase 5 — ストレージ・図管理 ✅ 完了
- [x] #38 ローカルファイル保存機能の実装（JSON 形式）
- [x] #39 ブラウザ LocalStorage 保存機能の実装
- [x] #40 両ストレージ間の同期ロジックの実装
- [x] #41 図一覧表示コンポーネントの実装
- [x] #42 図の検索・タグフィルタ機能の実装
- [x] #43 図の読み込み・再編集機能の実装
- [x] #44 図の削除機能の実装
- [x] #45 1000 件保存時のパフォーマンステスト

### Phase 6 — PDF 出力 ✅ 完了
- [x] #46 Puppeteer / Chromium の導入・設定
- [x] #47 単一図の PDF 出力機能の実装
- [x] #48 複数図をまとめた PDF 出力機能の実装
- [x] #49 PDF 表紙（タイトル・日付・説明）の実装
- [x] #50 A4 縦横対応の確認・調整
...
- #74 AI生成フェーズ表示（生成中スピナー・完了・エラー状態のボタンUI、エラーメッセージ表示）
...
| 機能 | 場所 | Phase |
|------|------|-------|
| PDF 出力 | `page.tsx: handleExportPdf` | Phase 6 |

---

## Phase 5 以降の追加実装（Issue 外）

- 左パネル各セクション（図の種類・要件入力・保存済み図）をタイトルクリックで開閉
- 保存済み図をフォルダでグループ管理・移動（📁 ポップオーバー）
- 保存済み図を更新日時・名前・種別でソート
- 保存ダイアログに Enter2回押しでの確定防止（誤保存対策）
- 保存ダイアログにフォルダ入力欄追加（datalist で既存フォルダ補完）
- Mermaid ノードラベルの日英混在テキスト折り返し修正（Safari 対応）
  - DOMPurify が `<div>` を strip するため、JS で XHTML 名前空間 div を挿入
  - flowchart padding を 15→24 に拡大
